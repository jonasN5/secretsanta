...
def assert_lists_equal(list1, list2, unsorted: bool = True, unsorted_lists: List[str] = None) -> None:
    """Compare two lists of dictionaries. If unsorted is True, the lists are compared without regard to order.
    Requires that the dictionaries in the lists have an 'id' key.
    @param list1: the first list
    @param list2: the second list
    @param unsorted: whether the lists should be compared without regard to order
    @param unsorted_lists: same as compare_dicts to be passed recursively
    """
    if unsorted:
        list2_by_id = {item['id']: item for item in list2}
    for index, item in enumerate(list1):
        if unsorted:
            assert_dicts_equal(item, list2_by_id[item['id']], unsorted_lists)
        else:
            assert_dicts_equal(item, list2[index], unsorted_lists)


def assert_dicts_equal(dict1, dict2, unsorted_lists: List[str] = []) -> None:
    """Compare two dictionaries containing unsorted lists. If a value is a dictionary, compare it recursively. If a
    value is a list, compare it with compare_lists.
    @param dict1: the first dictionary
    @param dict2: the second dictionary
    @param unsorted_lists: a list of keys that should be compared without regard to order
    """
    for key, value in dict1.items():
        if isinstance(value, dict):
            return assert_dicts_equal(value, dict2[key], unsorted_lists)
        elif isinstance(value, list):
            if key in unsorted_lists:
                assert_lists_equal(value, dict2[key], True, unsorted_lists)
            else:
                assert_lists_equal(value, dict2[key], False, unsorted_lists)
        else:
            assert value == dict2[key], f'Expected {value} for {key} but got {dict2[key]}'


def build_nested_user(user: User):
    """Build a nested user dictionary for comparison."""
    return {
        'id': str(user.id),
